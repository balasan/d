...
function initMove() {
  ...
  function start(e){
    ...
    function move(e){
      ...
      var dx = currentX - x;
      var width = Math.min(dx + w, 14 / inchesToPixels - headSizePx);
...
function initD(dontAnimate) {
  ...
  data.forEach(function(candidate, i) {
    ...
    if (dontAnimate) {
      $("#" + id + " .shaft").css({
        width: candidate.size / inchesToPixels - headSizePx
      })
    }
    else animatePenis(candidate, i);
  })

  function animatePenis(candidate, i) {
    setTimeout(function() {
      var id = candidate.name.replace(" ", "_");
      $("#" + id + " .shaft").animate({
        width: candidate.size / inchesToPixels - headSizePx
